...
<button type="button" class="btn btn-primary btn-lg" onClick="window.location.href='securityUI.jsp'">Back</button>
			...
			String sql ="SELECT * FROM complaints WHERE complaint_status = 'claimed' AND complaint_category = 'security' AND complaint_claimed_by = "+ session.getAttribute("USER_ID") +" AND complaint_done_by = 0 ORDER BY complaint_id DESC";
			...
			<tr>
		    	...
		    	<td><a href='securityClaimedComplaints.jsp/security/unclaim/<%out.println(resultSet.getString("complaint_id")); %>'>Unclaim</a></td>
				<td><a href='securityClaimedComplaints.jsp/security/done/<%out.println(resultSet.getString("complaint_id")); %>/<%=session.getAttribute("USER_ID")%>'>Yes</a></td>
			</tr>
