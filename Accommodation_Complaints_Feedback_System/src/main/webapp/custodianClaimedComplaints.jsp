...
<button type="button" class="btn btn-primary btn-lg" onClick="window.location.href='custodianWorkspace.jsp'">Back</button>
			...
			String sql ="SELECT * FROM complaints WHERE complaint_status = 'claimed' AND complaint_category = 'custodian' AND complaint_claimed_by = "+ session.getAttribute("USER_ID") +" AND complaint_done_by = 0 ORDER BY complaint_id DESC";
			...
			<tr>
		    	...
		    	<td><a href='custodianClaimedComplaints.jsp/custodian/unclaim/<%out.println(resultSet.getString("complaint_id")); %>'>Unclaim</a></td>
				<td><a href='custodianClaimedComplaints.jsp/custodian/done/<%out.println(resultSet.getString("complaint_id")); %>/<%=session.getAttribute("USER_ID")%>'>Yes</a></td>
			</tr>
